...
Use this skill when defining NPC behavior through JSON templates. This covers the data-driven side of NPC creation — how NPCs think, act, detect threats, fight, interact with other NPCs, and transition between behavioral states. For programmatic NPC spawning via Java, see the `docs/hytale-api/spawning-npcs.md` skill instead.
...
### New NPC Conditions (2026.03.26)

Three new conditions for NPC decision-making:

| Condition | What It Checks |
|-----------|---------------|
| `SelfHasEffectCondition` | Does this NPC have a specific EntityEffect active? |
| `TargetHasEffectCondition` | Does the NPC's target have a specific EntityEffect active? |
| `CheckPlayerGameModeCondition` | Is the target player in a specific GameMode? |
| `IsPlayerCondition` | Is the target entity a player (has Player component)? |
| `InFluidCondition` | Is the entity overlapping fluid blocks? |

These expand the NPC behavior tree with effect-awareness (e.g., NPCs can react differently to burning targets) and player-type awareness.
...
Interactive NPCs (merchants, quest givers, dialog NPCs) use a **second behavior tree** called `InteractionInstruction` alongside the main `Instructions` tree. This section covers how they work together, using the vanilla Kweebec Merchant as reference.

> **Source:** [Interactable NPCs guide](https://hytalemodding.dev/en/docs/guides/plugin/Interactable-NPCs) by VibeTheory

### Key Properties for Interactive NPCs

| Field | Value | Purpose |
|-------|-------|---------|
| `DefaultPlayerAttitude` | `Neutral` | Doesn't attack players |
| `Invulnerable` | `true` | Can't take damage |
| `BusyStates` | `["$Interaction"]` | Blocks other players from interacting while NPC is busy |

**`BusyStates` detail:** When the NPC enters `$Interaction`, the engine marks it as busy. Other players pressing F get rejected at the packet level (`willInteractWith()` returns false). Remove `$Interaction` from BusyStates to allow multiple players to interact simultaneously (each gets their own shop window).

### InteractionInstruction Tree

The `InteractionInstruction` tree is separate from `Instructions`. It runs **once per nearby player** (within ~10 blocks), every tick, and controls:
1. Whether the F-key prompt is shown
2. What happens when the player presses F

#### Instruction 1: Hide Prompt (facing check)
...
If the player is behind the NPC (outside the 180° view sector), hide the F prompt.

#### Instruction 2: Show Prompt
...
If we passed instruction 1 (player IS facing NPC), show the F prompt with the "trade" hint. `Continue` ensures instruction 3 also evaluates this tick.

#### Instruction 3: Handle F-key Press
...
`HasInteracted` fires once when F is pressed (consumes input). If the NPC is NOT already in `$Interaction`, three actions fire:
1. **LockOnInteractionTarget** — store this player as NPC's target for head tracking
2. **OpenBarterShop** — open shop UI (per-player, not shared)
3. **State: $Interaction** — transition to interaction state

### How the Two Trees Connect

1. `Instructions`: Player approaches → NPC waves, transitions to **Watching**
2. `InteractionInstruction`: Shows F prompt if player is facing the NPC
3. Player presses F → `HasInteracted` fires → locks target, opens shop, sets **$Interaction**
4. `Instructions`: Sees **$Interaction** → watches player, starts 1-second timeout
5. Timeout expires → releases target, returns to **Watching**

### The $Interaction State (in Instructions tree)
...
The 1-second delay prevents the NPC from snapping to idle the instant the player closes the shop.

### Custom NPC Actions

Custom actions are registered with `NPCPlugin.get().registerCoreComponentType()` — the same API that `OpenBarterShop` uses internally.

**Registration:**
```java
NPCPlugin.get().registerCoreComponentType("MyCustomAction", MyCustomAction::new);
```

Then reference in NPC role JSON:
```json
{ "Type": "MyCustomAction", "YourField": "your_value" }
```
...
| Action | Purpose |
|--------|---------|
| `OpenBarterShop` | Open a barter shop UI |
| `SetInteractable` | Show/hide F-key prompt |
| `LockOnInteractionTarget` | Lock NPC's target to interacting player |
| `ReleaseTarget` | Release locked target |

### InteractionInstruction Sensors

| Sensor | Description |
|--------|-------------|
| `CanInteract` | Player is within the NPC's view sector. `ViewSector`: angle in degrees |
| `HasInteracted` | Player pressed F (consumes the input — fires once) |

### Design Tips

- **Single-player interaction**: Keep `$Interaction` in `BusyStates`
- **Multi-player interaction**: Remove `$Interaction` from `BusyStates` (shop is per-player anyway)
- **Dialog trees**: Replace `OpenBarterShop` with a custom action that opens a HyUI page
- **Quest givers**: Use custom actions to check/grant quests on F interact
...
- `docs/hytale-api/spawning-npcs.md` — Programmatic NPC spawning via Java (NPCPlugin API)
- `docs/hytale-api/ecs.md` — Entity Component System patterns
- `docs/hytale-api/items.md` — Item registry, ItemStack, and interactions
- `docs/hytale-api/entity-effects.md` — Status effects and buffs
- `docs/hytale-api/events.md` — Event system for reacting to NPC-related events
