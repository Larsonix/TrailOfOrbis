...
> **Related skills:** For block *components* and ECS ticking behavior, see `docs/hytale-api/ecs.md`. For items and interactions, see `docs/hytale-api/items.md`.
...
| Set block animation | `"CustomModelAnimation": "Blocks/Animations/<name>.blockyanim"` |
...
Blocks can have animated textures using `.blockyanim` files. This is done through the `CustomModelAnimation` property in the BlockType JSON.

> **Source:** [Animated Block Textures guide](https://hytalemodding.dev/en/docs/guides/plugin/animated-block-textures) by Burkino

### Required Assets

| Asset | Path | Purpose |
|-------|------|---------|
| Texture | `Common/VFX/<Name>/<Name>.png` | Sprite sheet or animated texture |
| Model | `Common/VFX/<Name>/<Name>.blockymodel` | 3D model for the block |
| Animation | `Common/Blocks/Animations/<Name>/<Name>.blockyanim` | Animation keyframes |

### BlockType Properties for Animation

| Property | Type | Description |
|----------|------|-------------|
| `DrawType` | `String` | Must be `"Model"` for animated blocks |
| `CustomModel` | `String` | Path to `.blockymodel` file |
| `CustomModelAnimation` | `String` | Path to `.blockyanim` file |
| `CustomModelTexture` | `Array` | Texture + weight pairs |
| `Looping` | `Boolean` | Whether animation loops continuously |
| `RequiresAlphaBlending` | `Boolean` | Enable for transparent textures |

### Example: Animated Fire Block
...
YourMod/
├── Common/
│   ├── Blocks/Animations/<Name>/
│   │   └── <Name>.blockyanim          # Animation file
│   └── VFX/<Name>/
│       ├── <Name>.blockymodel         # 3D model
│       └── <Name>.png                 # Texture
└── Server/Item/Items/
    └── <Name>.json                    # Block/item definition
```

### Tips

- Copy existing vanilla animation files (e.g., `Common/Blocks/Animations/Fire/Fire_Burn.blockyanim`) as starting points
- `Looping: true` is required for continuous animations (fire, water, etc.)
- Animated blocks can be combined with block states for state-dependent animations

---

## BlockReplaceEvent (New in 2026.03.26)

ECS event fired when a block at a position is replaced (by player building, world gen, or game logic).
...
|--------|---------|-------------|
| `getChunkRef()` | `Ref<ChunkStore>` | Reference to the chunk containing the block |
| `getSelfX/Y/Z()` | `int` | Block position within the chunk |
| `getNewEntity()` | `Holder<ChunkStore>` | Holder for the new block entity |

**Use case:** Track block changes in realm instances (logging, persistence, detecting player builds).
