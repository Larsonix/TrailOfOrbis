...
> **Related skills:** For persistent player data, see `docs/hytale-api/persistent-data.md`. For notifications with item icons, see `docs/hytale-api/notifications.md`. For hotbar key actions, see `docs/hytale-api/hotbar-actions.md`. For inventory change events, see `docs/hytale-api/events.md`.
...
| Method | Combines |
|--------|----------|
| ~~`.getCombinedEverything()`~~ | ~~All containers~~ (**Removed** in 2026.03.26 — use `.getCombinedArmorHotbarUtilityStorage()` instead) |
...
| Event | Fires When |
|-------|------------|
| `InventoryChangeEvent` | An entity's inventory changes (**Updated 2026-03-26**: Renamed from `LivingEntityInventoryChangeEvent`. Now an ECS event — requires `EntityEventSystem` registration, not `EventRegistry.registerGlobal()`. See [events.md](events.md#ecs-events).) |
...
2. **Use combined containers for searches** — When checking if a player has an item, use `getCombinedArmorHotbarUtilityStorage()` or the appropriate combined method. (**Updated 2026-03-26**: `getCombinedEverything()` was removed; also note `InventoryComponent.markDirty()` replaces `Inventory.markChanged()`.)
