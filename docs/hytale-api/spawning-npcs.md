...
> Prerequisite: Familiarity with the Entity Component System (ECS). See the `docs/hytale-api/ecs.md` skill.
...
Pair<Ref<EntityStore>, INonPlayerCharacter> result = NPCPlugin.get().spawnNPC(
    ...
    position,           // Vector3d position to spawn at
...
- `docs/hytale-api/items.md` — Item registry and ItemStack usage
- `docs/hytale-api/commands.md` — Command creation patterns
