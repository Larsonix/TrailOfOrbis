...
> **Related skills:** For hotbar-specific slot customization (ability slots), see `docs/hytale-api/hotbar-actions.md`. For game events (PlayerReady, chat, damage, etc.), see `docs/hytale-api/events.md`. For UI-based input, see `docs/hytale-api/native-ui.md`.
...
> **Common input triggers:** `Primary` (left click), `Secondary` (right click), `Use` (F key). For hotbar slot-based ability triggers, see the `docs/hytale-api/hotbar-actions.md` skill.
